...
!begin   WSO 5/11/23 corrected code bug where non-MS input were used on rhs of equation
            pwc3DstdMS(k,i,j)=sqrt(sum((dPRRet%MS%pwc(1:nmemb1,k,i,j)-            &
                 pwc3DMS(k,i,j))**2)/(nmemb1-1))
!end     WSO 5/11/23
...
!end    WSO 9/17/13
      ...
      if(ialg==2) then
         ...
         call copy_tot_to_liqrate_ku_t(i-1, rrate3D(:,i,j), dPRData%node(:,i,j))
         call copy_tot_to_liqwatercont_ku_t(i-1, pwc3D(:,i,j), dPRData%node(:,i,j))
         ...
         call copyd0_a_t(dm3d_a(:,i,j),i-1)  ! apriori dm; altered for TRMM 2/24/22
...
!end    WSO 8/19/13
      ...
      if(ialg==2) then 
         ...
         call copyw10_t(w10_out_NS(i,j),i-1) !modified SJM 12/4/2014
         call copyw10sigma_t(w10_rms_NS(i,j),i-1) !modified SJM 12/4/2014
 ...
 !        call copy_oe_missing_staff_t(i-1)
      else
