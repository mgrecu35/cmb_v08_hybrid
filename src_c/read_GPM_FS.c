...
TKINFO dprtkfile;
extern L2ADPR_SWATHS_7h0 dprxswath;
...
//aa  begin  SFM  09/04/2013
    ...
    status = TKopen(&fname[0], "2AKu_7g0", TKREAD, "HDF5", jobname,
		    &granuleHandle2AKu, 1);
...
//  SFM  begin 12/12/2013; to pass out TK status message
	if(strncmp(dprfname,"nil",3)!=0 && *status_alpha == 0)
	  {
//aa  begin  SFM  09/04/2013
	    *status_alpha = TKopen(dprfname,"2ADPR_7h0",TKREAD,"HDF5",jobname,&dprtkfile,1);
...
//		  &granuleHandle2AKu, 1);
  printf("%s\n",&fname[0]);
  printf("got here\n");
  status_alpha = TKopen(&fname[0], "2AKu_7g0", TKREAD, "HDF5", jobname,
		  &granuleHandle2AKu, 1);
  if (status_alpha != 0)
      {
