subroutine gmiretsub(icL,ichunk,orbNumb,ialg,idir)
  ...
  real :: testlat, testlon


  !!temporary storage for variables interpolated to DPR footprint
  !real, dimension(:,:), allocatable :: dprSknTemp, dprSfcTemp, dprSfcWind

!begin WSO 11/17/21 check for bad lats/lons, and output flags if found
  testlat = minval(gmidata%S1lat(:, :))
  testlon = minval(gmidata%S1lon(:, :))
  print*, "testlat: ", testlat, "  testlon: ", testlon
  if(testlat < missing_r4 + 1. .or. testlon < missing_r4 + 1.) then  !bad geolocation
     write(*, '("size of OEQv: ", 10i10)') size(dprdata%OEQv)
!load OE arrays with flag values
    dprdata%OEQv(:, :, :) = missing_r4
    dprdata%OETemp(:, :, :) = missing_r4
    dprdata%OECloud(:, :, :) = missing_r4
    dprdata%OECloudLiqPath(:, :) = missing_r4
    dprdata%OEcloudLiqSigma(:, :) = missing_r4
    dprdata%OESfcWind(:, :) = missing_r4
    dprdata%OESfcWindSigma(:, :) = missing_r4
    dprdata%OESknTemp(:, :) = missing_r4
    dprdata%OEskinTempSigma(:, :) = missing_r4
    dprdata%OESfcTemp(:, :) = missing_r4
    dprdata%OESfcQv(:, :) = missing_r4
    dprdata%OEtpw(:, :) = missing_r4
    dprdata%OEtpwSigma(:, :) = missing_r4
    dprdata%OEchiSq(:, :) = missing_r4
    dprdata%OEpiaNonRain(:, :, :) = missing_r4
    dprdata%OEsimTbNonRain(:, :, :) = missing_r4
    dprdata%OEemis(:, :, :) = missing_r4
    dprdata%OEemisSigma(:, :, :) = missing_r4
    dprdata%OEemisA(:, :, :) = missing_r4
    dprdata%OEstype(:, :) = missing_i4
!modified environmental variables
    dprdata%modQv(:, :, :) = missing_r4
    dprdata%modTemp(:, :, :) = missing_r4
    dprdata%modCloud(:, :, :) = missing_r4
    dprdata%modSfcWind(:, :) = missing_r4
    dprdata%modSfcWindU(:, :) = missing_r4
    dprdata%modSfcWindV(:, :) = missing_r4
    dprdata%modSknTemp(:, :) = missing_r4
    dprdata%modSfcTemp(:, :) = missing_r4
    write(*, '("max value of OEQv: ", f15.6)') maxval(dprdata%OEQv)
    return
  endif
!end WSO 11/17/21
  ...
  print*, "ndpr: ", ndpr, "  dprdata%n1c21: ", dprdata%n1c21, "  icL: ", icL, "  gmidata%n1b11: ", gmidata%n1b11
  ...
  print*, "first minGMIscan(1): ", minGMIscan(1), "  first maxGMIscan(1): ", maxGMIscan(1)
  ...
  print*, "minGMIscan: ", minGMIscan, "  maxGMIscan: ",  maxGMIscan, "  minGMIpix: ", minGMIpix, "  maxGMIpix: ", maxGMIpix
...
!diagnostic
!      if(gmi_ret(j2, i2)%tpw_ret .ne. gmi_ret(j2, i2)%tpw_ret) then
!        write(*, '("NaN in GMI retrieval i: ", i5, "  j: ", i5, "  i2: ", i5, "  j2: ", i5, &
!         "  chiocean: ", e15.6, "  chiland: ", e15.6, "  chiice: ", e15.6)') &
!         i, j, i2, j2, gmi_ret(j2,i2)%chisq_ocean, gmi_ret(j2,i2)%chisq_land, gmi_ret(j2,i2)%chisq_ice
!      endif
!end diagnostic
...
!diagnostic
!      if(gmi_ret(j2, i2)%tpw_ret .ne. gmi_ret(j2, i2)%tpw_ret) then
!        write(*, '("NaN weighting i: ", i5, "  j: ", i5, "  i2: ", i5, "  j2: ", i5, "  gmi_wgt: ", e15.6)') &
!         i, j, i2, j2, gmi_wgt(j2, i2)
!      endif
!end diagnostic
...
!diagnostic
!  do idpr = 1, gmidata%n1b11
!    do jdpr = 1, 221
!       if(idpr <= 100) then
!         write(*, '("idpr: ", i10, "  jdpr: ", i10, "   S1lat: ", f10.4, &
!          "  S1lon: ", f10.4)') idpr, jdpr, gmidata%S1lat(jdpr, idpr), gmidata%S1lon(jdpr, idpr)
!       endif
!    enddo
!  enddo
!end diagnostic
...
!diagnostic
!          if(minGMIscan(1) .eq. 0 .or. maxGMIscan(1) .eq. 0) then
!             print*, "dprdata%n1c21: ", dprdata%n1c21
!             print*, "where S1lon arg out of bounds, idpr: ", idpr, "  jdpr: ", jdpr, "  minGMIscan(1): ", minGMIscan(1), "  maxGMIscan(1): ", maxGMIscan(1), &
!              "  minGMIpix(1): ", minGMIpix(1), "  maxGMIpix(1): ", maxGMIpix(1)
!          endif
!end diagnostic
...
!diagnostic
!      if(dprdata%OEtpw(jdpr, idpr) .ne. dprdata%OEtpw(jdpr, idpr)) then
!        write(*, '("NaN in assign to dprdata gloc(1): ", i5, "  gloc(2): ", i5, "  idpr: ", i5, "  jdpr: ", i5)') &
!         gloc(1), gloc(2), idpr, jdpr
!      endif
!end diagnostic
