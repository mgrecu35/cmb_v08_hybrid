...
!...Local computational variables
    ...
    CHARACTER(len=20) :: jkalgid
    type (TKINFO), pointer  :: granuleHandleL2 => NULL()
   
    print*, 'In header'

    print* , 'length of algorithm_ID ', len(algorithm_ID)
    pos = index(filename, "ENV")
    if(pos > 0) then
       ret = TKgetAlgorithmVersion(dummy_jobid, filename, jkalgid)
       print*,'from TKgetAlgorithmVersion jkalgid ', jkalgid
       allocate(granuleHandleL2)
       ret = TKopen(filename,jkalgid,TKREAD, "HDF5", dummy_jobid,granuleHandleL2,1)
       algorithm_ID = jkalgid
    else
       ret = TKgetAlgorithmVersion(dummy_jobid, filename, algorithm_ID)
       print*,'from TKgetAlgorithmVersion algorithm_ID ',algorithm_ID
       allocate(granuleHandleL2)
       ret = TKopen(filename,algorithm_ID,TKREAD, "HDF5", dummy_jobid,granuleHandleL2,1)
    endif

    print*,'algid from TKINFO struct ',granuleHandleL2%algid
    print*,'algid from non ENV ', algorithm_ID
    print*,'algid from ENV ', jkalgid
