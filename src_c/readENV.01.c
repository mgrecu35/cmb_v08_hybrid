...
#endif
#include "TK_2AKuENV_7b0.h"
//  Begin  SFM  09/12/2013
extern TKINFO ctkfile;
//  end  SFM  09/12/2013
//  begin  SFM  09/04/2013
L2AKuENV_FS_7b0        L2AKuENVDatax;
L2APRENV_FS        L2AKuENVData; 
...
//                      jobname, &granuleHandle2AKuENV, 1);
      ...
      if(*ialg == 2) 
       {
        ...
        printf("TKopen() 2AKuENV\n");
        status_alpha = TKopen(envfname, "2AKuENV_7b0", TKREAD, "HDF5",  jobname, &granuleHandle2AKuENV, 1);
       }
    if (status_alpha != 0)
      {
    ...
    printf("opened ENV file %s %d \n",&f2akuENV[0],status_alpha);
...
//   in rain areas [0], but later we went to option [1] for Chris' radiometer simulations
...
//diagnostic
//        if(j == 24)
//         printf("j: %5i, angle:  %8.4f,  k: %5i,  envTemp: %10.2f,  envPress: %12.2f %12.2f %12.2f,  envQv: %12.4f %12.4f %12.4f,  envCloud: %12.4f %12.4f %12.4f\n",
//           j, ray_angle, k, envTemp[i3d], 
//           L2AKuENVDatax.VERENV.airPressure[j][2*k], L2AKuENVDatax.VERENV.airPressure[j][2*k+1], envPress[i3d], 
//           L2AKuENVDatax.VERENV.waterVapor[j][2*k][1] * 1.e+3, L2AKuENVDatax.VERENV.waterVapor[j][2*k+1][1] *1.e+3, envQv[i3d], 
//           L2AKuENVDatax.VERENV.cloudLiquidWater[j][2*k][1] * 1.e+3, L2AKuENVDatax.VERENV.cloudLiquidWater[j][2*k+1][1] *1.e+3, envCloud[i3d]);
//end diagnostic
...
int readenvt_(char *jobname, int *ialg, char *f2akuENV, int *n1c21, int *ic, 
             int *nBSize, float *envQv, float *envTemp,
	     float *envPress, float *envSfcWind, float *envSfcWindU, float *envSfcWindV, float * envSknTemp, 
	     float *envSfcTemp, float *envSfcPress, float *envCloud)
{
...
//                      jobname, &granuleHandle2AKuENV, 1);
      ...
      if(*ialg == 2)
        ...
        status_alpha = TKopen(&f2akuENV[0], "2AKuENV_7b0", TKREAD, "HDF5",  jobname, &granuleHandle2AKuENV, 1);
...
//...for proper units of [g/m3]
//   note here, Mircea opts to use the water vapor adjusted by the algorithm
//   in rain areas [0], but later we went to option [1] for Chris' radiometer simulation
         if(L2AKuENVData.VERENV.waterVapor[j][2*k][1] >= 0. && L2AKuENVData.VERENV.waterVapor[j][2*k+1][1] >= 0.)
	        envQv[i3d] = 0.5 * (L2AKuENVData.VERENV.waterVapor[j][2*k][1] + L2AKuENVData.VERENV.waterVapor[j][2*k+1][1]) * 1.e+3;
         else if (L2AKuENVData.VERENV.waterVapor[j][2*k][1] >= 0.)
           envQv[i3d] = L2AKuENVData.VERENV.waterVapor[j][2*k][1] * 1.e+3;
         else if (L2AKuENVData.VERENV.waterVapor[j][2*k+1][1] >= 0.)
           envQv[i3d] = L2AKuENVData.VERENV.waterVapor[j][2*k+1][1] * 1.e+3;
         else
           envQv[i3d] = L2AKuENVData.VERENV.waterVapor[j][2*k][1];
//assume that variable contains flag value as final option

         if(L2AKuENVData.VERENV.cloudLiquidWater[j][2*k][1] >= 0. && L2AKuENVData.VERENV.cloudLiquidWater[j][2*k+1][1] >= 0.)
  	        envCloud[i3d] = 0.5 * (L2AKuENVData.VERENV.cloudLiquidWater[j][2*k][1] + 
                 L2AKuENVData.VERENV.cloudLiquidWater[j][2*k+1][1]) * 1.e+3;
         else if(L2AKuENVData.VERENV.cloudLiquidWater[j][2*k][1] >= 0.)
           envCloud[i3d] = L2AKuENVData.VERENV.cloudLiquidWater[j][2*k][1] * 1.e+3;
         else if(L2AKuENVData.VERENV.cloudLiquidWater[j][2*k+1][1] >= 0.)
           envCloud[i3d] = L2AKuENVData.VERENV.cloudLiquidWater[j][2*k+1][1] * 1.e+3;
         else
           envCloud[i3d] = L2AKuENVData.VERENV.cloudLiquidWater[j][2*k][1];
//assume that variable contains flag value as final option
...
//           L2AKuENVData.VERENV.waterVapor[j][2*k][1] * 1.e+3, L2AKuENVData.VERENV.waterVapor[j][2*k+1][1] *1.e+3, envQv[i3d], 
//           L2AKuENVData.VERENV.cloudLiquidWater[j][2*k][1] * 1.e+3, L2AKuENVData.VERENV.cloudLiquidWater[j][2*k+1][1] *1.e+3, envCloud[i3d]);
//end diagnostic
